...
public class PlayerControllerOnline : MonoBehaviourPunCallbacks
{
    ...
    public static PlayerControllerOnline instance;
    ...
    void Update()
    {
        ...
        if (currentGunIndex == 1)   //Shot repeating gun con surriscaldamento
        {
            if (!isOverHeated)
            {
                if (isFiring)
    ...
    public void SwitchGun()
    {
        ...
        if (currentGunIndex < allGuns.Count)            //allGuns.Count
        {
            currentGunIndex++;
        }
        else
